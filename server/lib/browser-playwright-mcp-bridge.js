...
async function tryDomClick(command = {}) {
  ...
  return {
    ...
    text: parsed
      ? "Field verification " + (parsed.ok ? "passed" : "failed") + ". " + safeText(JSON.stringify(parsed), 3000)
      : result.text,
    error: parsed?.ok === true ? "" : result.error || "Field verification did not confirm requested values.",
  };
