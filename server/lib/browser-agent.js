...
import { runBrowserAgentOrchestrator as runRouteIsolatedBrowserAgentOrchestrator } from "./browser-agent/orchestrator.js";
import {
  defaultBrowserAgentState as routeIsolatedDefaultBrowserAgentState,
  loadBrowserAgentState as routeIsolatedLoadBrowserAgentState,
  resetBrowserAgentState as routeIsolatedResetBrowserAgentState,
...
export async function browserAgentObserve(args = {}) {
  const sessionId = safeSessionId(args.sessionId || DEFAULT_SESSION_ID);
  const currentState = routeIsolatedLoadBrowserAgentState(sessionId);
  return runRouteIsolatedBrowserAgentOrchestrator({
    ...args,
    sessionId,
    currentState,
    instruction: String(args.instruction || "Observe the current page and report what is visible.").trim(),
  });
}
...
export async function browserAgentReset(args = {}) {
  const sessionId = safeSessionId(args.sessionId || DEFAULT_SESSION_ID);
  routeIsolatedResetBrowserAgentState(sessionId);
  return {
    ok: true,
    status: "success",
    sessionId,
    state: routeIsolatedDefaultBrowserAgentState(sessionId),
...
export async function browserAgentStatus(args = {}) {
  const sessionId = safeSessionId(args.sessionId || DEFAULT_SESSION_ID);
  const state = routeIsolatedLoadBrowserAgentState(sessionId);
  return {
    ok: true,
    status: "success",
    sessionId,
...
export async function browserAgentRun(args = {}) {
  return runRouteIsolatedBrowserAgentOrchestrator(args);
}
